...
function _monitorTransferProgress(stream, options, transferType, info, callback) {
  ...
  stream.on('data', function (chunk) {
    ...
    _emitTransferProgress.call(self, options, transferType, info, {read: totalRead, rate: rate});
  ...
  this.getInfo(options, function (err, info) {
    ...
    getStreamFunc.call(self, path, options, info, function (err, stream, contentType) {
      if (monitorProgress) {
        var totalRead = 0;
        var rate = 0;
        _emitTransferProgress.call(self, options, TRANSFER_TYPE_READ, info, {read: totalRead, rate: rate}, true);
        ...
        stream.on('end', function () {
          _emitTransferProgress.call(self, options, TRANSFER_TYPE_READ, info, {read: totalRead, rate: rate}, true, true);
        });
      }
...
function _getWriteStream(pathOrOptions, isCreate, readStream, callback) {
  ...
  _validate(function (err, info) {
    ...
    self._getAssetWriteStream(path, isCreate, options, info, function (err, writeStream) {
      ...
      _emitTransferProgress.call(self, options, transferType, progressInfo, {read: 0, rate: rate}, true, false);
      ...
      _emitTransferProgress.call(self, options, transferType, progressInfo, {read: totalRead, rate: rate}, true, true);
      if (callback) {
        self.getInfo(options, _sendCallback);
      }
    });
...
function _emitTransferProgress(options, transferType, info, progress, force, reset) {
  ...
  if (force || _shouldEmitProgress.call(this, path, transferType)) {
    this.emitCallback(options, function () {
      self.emit('transferprogress', {path: path, type: transferType, info: info, progress: progress});
    });
