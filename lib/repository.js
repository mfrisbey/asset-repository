...
/**
 * Retrieves the context id from options supplied to a method.
 * @param {String|Object} options A string or object containing options.
 * @returns {String} A context id value.
 */
Repository.getContextIdFromOptions = function (options) {
  return _getValueFromOptions(options, 'contextId', false);
};
...
/**
 * Checks whether or not a given set of options has a contextId. If missing, a new one will be added.
 * @param {String|Object} options A string or object containing options.
 * @returns {Object}
 */
Repository.ensureContextId = function (options) {
  var objType = (typeof options);
  if (objType === 'string') {
    options = {
      path: options
    };
  } else if (objType instanceof RegExp) {
    options = {
      searchTerm: options
    };
  }

  options.contextId = Repository.getContextIdFromOptions(options) || uuid();
  return options;
};
...
Repository.prototype.emitCallback = function (options, callback) {
  ...
  if ((!subscriberId) || this.isSubscribed(subscriberId)) {
    log.debug('subsriber is valid, emitting callback');
    callback();
  } else {
    log.debug('subsriber is invalid, ignoring callback');
  }
};
...
Repository.prototype.exists = function (pathOrOptions, callback) {
  ...
  pathOrOptions = Repository.ensureContextId(pathOrOptions);
  var log = Repository.getLogger(pathOrOptions);
...
Repository.prototype.findAssets = function (searchTermOrOptions, callback) {
  ...
  searchTermOrOptions = Repository.ensureContextId(searchTermOrOptions);
  var log = Repository.getLogger(searchTermOrOptions);
  var searchTerm = Repository.getSearchTermFromOptions(searchTermOrOptions);

  log.debug('finding assets %s', searchTerm);

  this._findAssets(searchTerm, Repository.getRawFromOptions(searchTermOrOptions), function (err, assets) {
    if (err) {
